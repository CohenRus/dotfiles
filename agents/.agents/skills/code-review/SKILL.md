---
name: Code Review
description: Perform a thorough code review covering functionality, maintainability, and security
---
...
- If a base branch was already specified, use it.
- Otherwise, check if `staging` exists (`git rev-parse --verify staging`). If it does, base is `staging`.
- If `staging` doesn't exist, base is `main`.
- If already on the base branch, diff against `HEAD` instead.

Run `git --no-pager diff <base>...HEAD` to get the changes for review.
**Always use `git --no-pager` to prevent the pager from taking over.**
Reference CLAUDE.md and cursor rules.

1. **Understand the change**
    - Use the git diff for context
